...
class JESCommandWindow(swing.JTextPane,
    ...
    def __init__(self, gui):
        ...
        self.setDocument(
            JESCommandWindowDocument.JESCommandWindowDocument(self))
        ...
        commandKeymap.addActionForKeyStroke(
            KeyStroke.getKeyStroke('UP'), makeAction(self.up))
        commandKeymap.addActionForKeyStroke(
            KeyStroke.getKeyStroke('DOWN'), makeAction(self.down))
        ...
        # initialize heldText.  heldText holds the last text to be copied or
        # cut.
        self.heldText = ""
...
###############################################################################
    ...
    def enterHelper(self):
        ...
        beginsWithEllipsis = self.doesLineBeginWithEllipsis(
            line)  # added DNR 10.24.02
...
##########################################################################
    def restoreConsole(self, mode):
        ...
        # 5/14/09 Dorn: added in a condition for LOAD since we added a notice
        # of load on the console
        elif mode == "load":
            self.showText(">>> ")
            self.oldPos = self.document.getLength()
        else:
...
##########################################################################
    def undo(self):
        ...
        if length - self.oldPos > 0:
            self.oldText = self.document.getText(
                self.oldPos, (length - self.oldPos))
            self.document.remove(self.oldPos, (length - self.oldPos))
    ...
    def redo(self):
        ...
        if length - self.oldPos == 0:
            self.document.insertString(
                self.currentPos, self.oldText, self.document.textAttrib)
            self.oldText = ''
...
def makeAction(obj):
    if isinstance(obj, JESAction):
        return obj
    if callable(obj):
        return JESAction(obj)
